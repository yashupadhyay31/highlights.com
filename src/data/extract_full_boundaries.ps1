$pathCountries = 'C:\Users\Yash Upadhyay\.gemini\antigravity-ide\brain\e5c9b22c-5634-44f2-823e-6dba6cfbcb78\.system_generated\steps\133\content.md'
$pathStates = 'C:\Users\Yash Upadhyay\.gemini\antigravity-ide\brain\e5c9b22c-5634-44f2-823e-6dba6cfbcb78\.system_generated\steps\147\content.md'

$rawCountries = Get-Content $pathCountries -Raw
$patternRing = '\[\[-?\d+\.?\d*,-?\d+\.?\d*\](?:,\[-?\d+\.?\d*,-?\d+\.?\d*\])+\]'

$matchesC = [regex]::Matches($rawCountries, $patternRing)
$countryRings = @()
foreach ($m in $matchesC) {
    $countryRings += $m.Value
}

$rawStates = Get-Content $pathStates -Raw
$matchesS = [regex]::Matches($rawStates, $patternRing)
$stateRings = @()
foreach ($m in $matchesS) {
    $stateRings += $m.Value
}

Write-Host "Extracted $($countryRings.Count) exact country boundary rings"
Write-Host "Extracted $($stateRings.Count) exact state/provincial boundary rings"

$output = @"
// Official Natural Earth 1:110m & 1:50m Real-World Boundary Polylines (Public Domain)
// Exact boundary shapes matching official geospatial world maps.
...
Write-Host "bordersData.js successfully generated with official real-world Natural Earth boundaries!"
